    ...
    <div class="col-md-3 col-xxl-2 mb-3">
        <div class="card overflow-hidden position-relative h-100">
            <div class="d-flex d-md-block align-items-center">
                ...
                @if($item->offsale_at != null && $item->special == 0 && !$item->offsale_at->isPast() && (($item->cash > 0 && $item->coins > 0) || ($item->coins < 0 && $item->cash < 0))) <div class="timed-badge"></div> @endif
                <a href="{{ route('market.item', $item->id) }}">
                    <img src="{{ $item->get_render() }}" class="img-fluid p-2 item-preview d-none d-md-block @if($item->special) is-collectible @endif @if($item->offsale_at != null && $item->special == 0 && !$item->offsale_at->isPast() && (($item->cash > 0 && $item->coins > 0) || ($item->coins < 0 && $item->cash < 0))) is-timed @endif" />
                    <img src="{{ $item->get_render() }}" class="item-preview p-2 d-block d-md-none @if($item->special) is-collectible @endif @if($item->offsale_at != null && $item->special == 0 && !$item->offsale_at->isPast() && (($item->cash > 0 && $item->coins > 0) || ($item->coins < 0 && $item->cash < 0))) is-timed @endif" width="128" />
                </a>
